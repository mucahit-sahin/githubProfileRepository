class UI {
  constructor() {
    this.profileRepositories = document.querySelector(".profileRepositories");
    this.repoList = [];
  }
  showUser(data) {
    ...
    if (data.bio !== null && data.bio !== "") {
      ...
      bio.classList.add("bioText");
      bio.appendChild(document.createTextNode(data.bio));
      if (document.querySelector(".bioText")) {
        document
          .querySelector(".profileBio")
          .replaceChild(bio, document.querySelector(".bioText"));
      } else {
        document
          .querySelector(".profileBio")
          .insertBefore(bio, document.querySelector(".editProfileButton"));
      }
    }
  ...
  showRepositories(data) {
    this.repoList = data;
    this.profileRepositories.innerHTML = "";
    data.map((repo) => {
      ...
      this.profileRepositories.innerHTML += repositoryElement;
    });
  }
  filterRepositories(e) {
    let pattern = new RegExp(e.target.value);
    var filterList = this.repoList.filter(function (repo) {
    ...
    if (filterList.length > 0) {
      ...
      this.profileRepositories.innerHTML = `<div class="notFound"><span>Repository Not Found</span></div>`;
    }
  }
}
